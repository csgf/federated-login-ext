...
boolean companySecurityLocalLogin = PrefsPropsUtil.getBoolean(company.getCompanyId(), FedPropsKeys.COMPANY_SECURITY_LOCAL_LOGIN, FedPropsValues.COMPANY_SECURITY_LOCAL_LOGIN);
...
<liferay-ui:tabs
	names="general,ldap,cas,facebook,ntlm,open-id,open-sso,siteminder,saml,stork"
	refresh="<%= false %>"
>
	<liferay-ui:section>
		<aui:fieldset>
			...
			<aui:input label="allow-local-login" name='<%= "settings--" + FedPropsKeys.COMPANY_SECURITY_LOCAL_LOGIN + "--" %>' type="checkbox" value="<%= companySecurityLocalLogin %>" />
